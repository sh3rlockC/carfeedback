...
from app.schemas import TaskCreateRequest, TaskCreateResponse, TaskDetailResponse, TaskListItem, TaskLoadResponse
...
from app.services.task_result_assembler import task_result_bundle_entries, task_result_payload
...
def _authorize_task_view(task: Task, request: Request, settings: Settings, view_token: str | None) -> None:
    if _has_passphrase_session(request, settings):
        return
    if not view_token:
...
def _resolve_downloadable_artifact_path(artifact: TaskArtifact, settings: Settings) -> Path:
    path = Path(artifact.path).expanduser().resolve()
    allowed_roots = (settings.artifact_root_path, settings.corpus_root_path)
...
def create_task(
    ...
    require_passphrase_session(request, settings)
    if not settings.task_center_create_enabled:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="task center creation is temporarily disabled; use the vehicle query flow",
        )
...
@router.get("/load", response_model=TaskLoadResponse)
def get_task_load(
    request: Request,
    db: Session = Depends(get_db),
    settings: Settings = Depends(get_settings),
...
def get_task(
    ...
    _authorize_task_view(task, request, settings, view_token)
...
def download_task_artifact(
    ...
    if not artifact.downloadable:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="artifact is not downloadable")
    path = _resolve_downloadable_artifact_path(artifact, settings)
...
def get_task_result(
    ...
    task = _load_task_or_404(db, task_id)
    _authorize_task_view(task, request, settings, view_token)
    return task_result_payload(settings, task)


@router.get("/{task_id}/artifacts.zip")
def download_task_artifact_bundle(
    task_id: str,
    request: Request,
    view_token: str | None = Query(default=None),
    db: Session = Depends(get_db),
    ...
    with zipfile.ZipFile(buffer, mode="w", compression=zipfile.ZIP_DEFLATED) as archive:
        for artifact, arcname in task_result_bundle_entries(task.artifacts):
            if not artifact.downloadable:
                continue
            try:
                path = _resolve_downloadable_artifact_path(artifact, settings)
            ...
            added += 1
    if added == 0:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="no downloadable task artifacts")
    buffer.seek(0)
    filename = f"{task.display_name}_结果包.zip"
    encoded_filename = quote(filename)
    return StreamingResponse(
        buffer,
        media_type="application/zip",
        headers={"Content-Disposition": f"attachment; filename=result.zip; filename*=UTF-8''{encoded_filename}"},
    )
...
def retry_task(
    ...
    task.status = "queued"
    task.current_stage = "generating_ai_report" if task.current_stage == "retry_paused" else "queued"
    db.add(TaskEvent(task_id=task.task_id, event_type="manual_retry_requested", payload_json={}))
    db.commit()
    if task.current_stage == "generating_ai_report":
        try:
            workflow_client.start_task(task.task_id, "single" if task.task_type == "single_vehicle" else task.task_type)
        except Exception as exc:
            db.add(TaskEvent(task_id=task.task_id, event_type="workflow_start_failed", payload_json={"error": str(exc)}))
            task.status = "retry_paused"
            task.current_stage = "retry_paused"
            db.commit()
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="task workflow unavailable") from exc
    db.refresh(task)
    return task_detail_payload(task)
...
@router.post("/{task_id}/degrade-result", response_model=TaskDetailResponse)
def degrade_task_result(
    task_id: str,
    request: Request,
    manage_token: str | None = Query(default=None),
    db: Session = Depends(get_db),
    ...
    task.status = "queued"
    task.current_stage = "generating_degraded_report"
    db.add(TaskEvent(task_id=task.task_id, event_type="degrade_result_requested", payload_json={}))
    db.commit()
    try:
        workflow_client.start_task(task.task_id, "single" if task.task_type == "single_vehicle" else task.task_type)
    except Exception as exc:
